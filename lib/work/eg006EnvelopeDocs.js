...
eg006EnvelopeDocs.createController = async (req, res) => {
    ...
    // double-check here to enable a better UX to the user.
        //array variable of Completed Envelopes from egGSEnvelopeList

    let arr = req.session.envelopeId;
    let ct = arr.length;
    //need get individial ids from array
    //check to see if arr is set and not null
    if (arr!=null){
        //for now getting one element
        req.session.envelopeId = arr[0]; // Save for use by other examples
    }
    ...
    if (! req.session.envelopeId) {
        res.render('pages/work/eg006EnvelopeDocs', {
    ...
    fs.readFile('./Ids.txt',{encoding: 'utf8'}, function (err,data) {
        if (!err){
