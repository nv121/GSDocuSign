 ...
 */
egGSEnvelopeList.createController = async (req, res) => {
    ...
    try {
        ...
        //await call to egGSEnvelopeDocs
...
//     };
egGSEnvelopeList.NewController = async (req, res) => {
    let args = {
        accessToken: req.user.accessToken,
        basePath: req.session.basePath,
        accountId: req.session.accountId,
    ...
    try {
        results = await egGSEnvelopeList.worker2 (args)
    }
    catch (error) {
        let errorBody = error && error.response && error.response.body
            // we can pull the DocuSign error code and message from the response body
...
egGSEnvelopeList.worker2 = async (worker) => {
    // Data for this method
    // args.basePath
    // args.accessToken
    // args.accountId
